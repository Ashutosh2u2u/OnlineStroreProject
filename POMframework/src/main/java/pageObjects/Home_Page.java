...
public class Home_Page extends BaseClass
{
	...
	public static WebElement warningRibbonDismiss() throws Exception{
		try{ 
			element = driver.findElement(By.xpath("//a[contains(text(),'Dismiss')]"));
			if(element.getText().contains("Dismiss")){
				element.click();
			Log.info("Warning Ribbon appear on top and Dismissed");
			}
		}catch (Exception e){
			Log.error("Warning Ribbon not appear on top and Dismissed");
			throw(e);
		}
		return element;
	}
